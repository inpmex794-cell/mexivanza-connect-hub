import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
import Stripe from "https://esm.sh/stripe@14.21.0";
import { createClient } from "https://esm.sh/@supabase/supabase-js@2.45.0";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type",
};

serve(async (req) => {
  // Handle CORS preflight requests
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }

  try {
    const { bookingData } = await req.json();
    
    // Create Supabase client using the anon key for user authentication
    const supabaseClient = createClient(
      Deno.env.get("SUPABASE_URL") ?? "",
      Deno.env.get("SUPABASE_ANON_KEY") ?? ""
    );

    // Authenticate user if auth header is present
    let user = null;
    const authHeader = req.headers.get("Authorization");
    if (authHeader) {
      const token = authHeader.replace("Bearer ", "");
      const { data } = await supabaseClient.auth.getUser(token);
      user = data.user;
    }

    // Initialize Stripe
    const stripe = new Stripe(Deno.env.get("STRIPE_SECRET_KEY") || "", {
      apiVersion: "2023-10-16",
    });

    // Get customer email - use user email or booking email
    const customerEmail = user?.email || bookingData.contact_info.email;

    // Check if a Stripe customer record exists
    const customers = await stripe.customers.list({ email: customerEmail, limit: 1 });
    let customerId;
    if (customers.data.length > 0) {
      customerId = customers.data[0].id;
    }

    // Get listing details for pricing
    const supabaseService = createClient(
      Deno.env.get("SUPABASE_URL") ?? "",
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") ?? "",
      { auth: { persistSession: false } }
    );

    const { data: listing, error: listingError } = await supabaseService
      .from('travel_bookings_categories')
      .select('*')
      .eq('id', bookingData.listing_id)
      .single();

    if (listingError || !listing) {
      throw new Error('Listing not found');
    }

    // Create product name based on category and listing
    const getProductName = (category: string, title: any) => {
      const categoryNames = {
        flights: 'Flight Booking',
        hotels: 'Hotel Reservation',
        insurance: 'Travel Insurance',
        cruises: 'Cruise Booking',
        airbnb: 'Accommodation Booking',
        tour_guides: 'Tour Guide Service',
        charters: 'Charter Service',
        car_rentals: 'Car Rental'
      };
      
      const categoryName = categoryNames[category as keyof typeof categoryNames] || 'Travel Service';
      const listingTitle = title?.en || title?.es || 'Service';
      return `${categoryName}: ${listingTitle}`;
    };

    // Create a one-time payment session
    const session = await stripe.checkout.sessions.create({
      customer: customerId,
      customer_email: customerId ? undefined : customerEmail,
      line_items: [
        {
          price_data: {
            currency: listing.price_currency?.toLowerCase() || "mxn",
            product_data: { 
              name: getProductName(listing.category, listing.title),
              description: listing.description?.en || listing.description?.es || 'Travel service booking'
            },
            unit_amount: Math.round(bookingData.total_amount * 100), // Convert to cents
          },
          quantity: 1,
        },
      ],
      mode: "payment",
      success_url: `${req.headers.get("origin")}/travel/booking-success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${req.headers.get("origin")}/travel/categories`,
      metadata: {
        category: listing.category,
        listing_id: bookingData.listing_id,
        user_id: user?.id || 'guest',
      },
    });

    // Create booking record in Supabase
    const { data: booking, error: bookingError } = await supabaseService
      .from("user_travel_bookings")
      .insert({
        user_id: user?.id,
        listing_id: bookingData.listing_id,
        category: listing.category,
        booking_data: bookingData.booking_data,
        total_amount: bookingData.total_amount,
        booking_dates: bookingData.booking_dates,
        contact_info: bookingData.contact_info,
        special_requests: bookingData.special_requests,
        stripe_session_id: session.id,
        booking_status: "pending",
        payment_status: "pending"
      })
      .select()
      .single();

    if (bookingError) {
      console.error("Booking creation error:", bookingError);
      throw new Error("Failed to create booking record");
    }

    return new Response(JSON.stringify({ 
      url: session.url,
      booking_id: booking.id 
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
  } catch (error) {
    console.error("Category booking error:", error);
    return new Response(JSON.stringify({ error: error.message }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
});